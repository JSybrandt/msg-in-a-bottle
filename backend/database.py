...
def open_login_request(email):
  ...
  existing_query = db.session.query(PendingLoginRequest).filter(PendingLoginRequest.email == email)
  if existing_query.first() is None:
    db.session.add(PendingLoginRequest(email=email, secret_key=secret_key, timestamp=util.now()))
  else:
